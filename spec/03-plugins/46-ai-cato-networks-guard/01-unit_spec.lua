...
describe(PLUGIN_NAME .. ": (cato_client url)", function()
...
describe(PLUGIN_NAME .. ": (cato_client interpret_request)", function()
  it("returns nil action when no required_action", function()
    local action = cato.interpret_request({ required_action = nil })
    assert.is_nil(action)
  end)

  it("treats monitor_action as allow", function()
    local action = cato.interpret_request({
      required_action = { action_type = "monitor_action" },
    })
    assert.equals("monitor_action", action)
  end)
